...
if (-not (Test-Path -LiteralPath $RuntimeSkillsPath)) {
    throw "Claude Code runtime skills path not found: $RuntimeSkillsPath"
}

if (-not $NoBackup) {
    $stamp = Get-Date -Format 'yyyyMMdd-HHmmss'
    $backupRoot = Join-Path $repoRoot "generated\claude-code\source-backups\$stamp"
...
foreach ($skill in $skills) {
    ...
    Write-Host "Synced Claude Code runtime skill into repo: $skill"
}

Write-Host "Claude Code sync complete. Source repo: $skillsRoot"
